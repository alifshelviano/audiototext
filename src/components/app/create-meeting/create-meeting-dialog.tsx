...
export function CreateMeetingDialog({ children }: CreateMeetingDialogProps) {
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="bg-blue-50 rounded-xl p-4 space-y-3">
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center gap-3 text-sm">
            <div className={`h-4 w-4 ${isPublic ? "text-green-600" : "text-amber-600"}`}>{isPublic ? <Globe /> : <Lock />}</div>
            <span className={`font-medium ${isPublic ? "text-green-700" : "text-amber-700"}`}>{isPublic ? "Public Meeting" : "Private Meeting"}</span>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-3">
        ...
        <div className="flex gap-2">
          <div className="flex-1 relative">
            <Input value={meetingLink} readOnly className="pr-20 bg-gray-50 border-gray-200" />
          </div>
          <Button onClick={handleCopy} variant={isCopied ? "default" : "outline"} className={`whitespace-nowrap ${isCopied ? "bg-green-600 hover:bg-green-700" : ""}`}>
            {isCopied ? (
              <>
  ...
  const CreateView = () => (
    <div className="space-y-6">
      ...
      <DialogHeader className="text-center pt-6 px-6">
        ...
        <DialogTitle className="text-xl font-bold text-gray-900">Create New Meeting</DialogTitle>
      ...
      <div className="px-6 space-y-5">
        ...
        <div className="flex items-center justify-between p-4 bg-gray-50 rounded-xl border">
          <div className="space-y-1">
            ...
            <p className="text-xs text-gray-600">{isPublic ? "Anyone with the link can join" : "Only invited participants can join"}</p>
          </div>
      ...
      <DialogFooter className="px-6 pb-6 pt-2">
        <Button
          ...
          className="w-full h-12 text-base font-semibold bg-gradient-to-r from-cyan-500 to-teal-700 hover:from-cyan-700 hover:to-teal-700 disabled:opacity-50 disabled:cursor-not-allowed"
        >
          {isCreating ? (
            <>
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-md p-0 overflow-hidden bg-white rounded-2xl shadow-2xl border-0">{creationSuccess ? <SuccessView /> : <CreateView />}</DialogContent>
    </Dialog>
  );
}
