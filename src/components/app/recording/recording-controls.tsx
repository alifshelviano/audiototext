...
// // // src/components/app/recording/recording-controls-with-socket.tsx
// // "use client";

// // import { useState, useRef, useCallback, useEffect } from "react";
// // import { useAuth } from "@/app/providers/AuthProvider";
// // import { addTranscriptToMeeting } from "@/lib/services/meeting-service";
// // import { transcribeAudioOpenAI } from "@/ai/flows/transcribe-audio-openai";
// // import { useSocket } from "@/hooks/use-socket";
// // import { Button } from "@/components/ui/button";
// // import { Mic, Square, Pause, Play, Users } from "lucide-react";
// // import { Badge } from "@/components/ui/badge";

// // interface RecordingControlsWithSocketProps {
// //   meetingId: string;
// //   onTranscriptAdded?: () => void;
// // }

// // export function RecordingControlsWithSocket({ meetingId, onTranscriptAdded }: RecordingControlsWithSocketProps) {
// //   const { user } = useAuth();
// //   const [isRecording, setIsRecording] = useState(false);
// //   const [isPaused, setIsPaused] = useState(false);
// //   const [isProcessing, setIsProcessing] = useState(false);
...
// //   const { isConnected, isReconnecting, on, sendTranscript, updateStatus, joinMeeting, leaveMeeting } = useSocket({
// //     meetingId,
// //     userName: user?.name,
// //     autoConnect: true,
// //   });
...
// //     const cleanup1 = on("transcript-added", (transcript) => {
...
// //     // Handle user joined
// //     const cleanup2 = on("user-joined", ({ userName }) => {
// //       console.log("User joined:", userName);
// //       setOnlineUsers((prev) => [...prev, userName]);
// //     });

// //     // Handle user left
// //     const cleanup3 = on("user-left", ({ userName }) => {
// //       console.log("User left:", userName);
// //       setOnlineUsers((prev) => prev.filter((u) => u !== userName));
// //     });

// //     // Handle typing indicator
// //     const cleanup4 = on("user-typing", (userName) => {
// //       setTypingUsers((prev) => {
...
// //     const cleanup5 = on("status-update", ({ userName, status }) => {
...
// //       updateStatus(meetingId, user.name, "idle");
// //     } else {
// //       updateStatus(meetingId, user.name, "online");
// //     }
// //   }, [isRecording, isPaused, isConnected, meetingId, user, updateStatus]);
...
// //         if (base64AudioData.length > 1000) {
// //           const { transcription } = await transcribeAudioOpenAI({
// //             audioDataUri: base64AudioData,
// //             language: "english",
// //           });
...
// //       setIsProcessing(false);
// //     }
// //   }, [meetingId, user, onTranscriptAdded, sendTranscript]);
...
// //           <span className="text-sm text-gray-600">{isConnected ? "Connected" : isReconnecting ? "Reconnecting..." : "Disconnected"}</span>
// //         </div>
...
// //           {typingUsers.join(", ")} {typingUsers.length === 1 ? "is" : "are"} speaking...
// //         </div>
// //       )}
...
// //         {!isRecording ? (
// //           <Button onClick={handleStartRecording} disabled={isProcessing || !isConnected} className="flex-1">
// //             <Mic className="w-4 h-4 mr-2" />
      ...
      <div className="bg-white rounded-xl p-4 border border-gray-200 shadow-sm">
        ...
        <div className="flex items-center justify-between gap-4">
          ...
          <div className="flex-shrink-0">
            {isRecording ? (
              <Button onClick={handleStopRecording} size="sm" variant="destructive" className="h-10 px-4 font-semibold">
                <Square className="w-4 h-4 mr-2" />
                Stop
              </Button>
            ) : (
              <Button onClick={handleStartRecording} size="sm" className="h-10 px-4 bg-blue-600 hover:bg-blue-700 font-semibold" disabled={isProcessing || !isConnected}>
                <Mic className="w-4 h-4 mr-2" />
                {isProcessing ? "Processing..." : "Start"}
              </Button>
            )}
