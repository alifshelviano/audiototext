...
const SummarizeTranscribedTextInputSchema = z.object({
  ...
  participants: z.array(z.string()).optional().describe("Array of actual participant names from the meeting"),
});
...
export const summarizeTranscribedTextFlow = ai.defineFlow(
  ...
  async (input: SummarizeTranscribedTextInput): Promise<SummarizeTranscribedTextOutput> => {
    ...
    // Use provided participants list (from actual users) or empty array
    const actualParticipants = input.participants || [];
    ...
    const languagePrompts = {
      english: {
        ...
        instructions: "Extract information only from the provided transcript and provide the analysis in English. Use the provided participants list.",
      },
      indonesian: {
        analysis: "Analisis transkrip rapat dan kembalikan HANYA objek JSON yang valid dalam struktur yang ditentukan di bawah ini.",
        instructions: "Ekstrak informasi hanya dari transkrip yang diberikan dan berikan analisis dalam Bahasa Indonesia. Gunakan daftar peserta yang disediakan.",
      },
      korean: {
        analysis: "회의 기록을 분석하고 아래의 정확한 구조로 유효한 JSON 객체만 반환하세요.",
        instructions: "제공된 기록에서만 정보를 추출하고 한국어로 분석을 제공하세요. 제공된 참가자 목록을 사용하세요.",
      },
    };
    ...
    const participantsString = actualParticipants.length > 0 
      ? actualParticipants.map(p => `"${p}"`).join(", ")
      : "[]";
...
## CONFIRMED PARTICIPANTS: [${participantsString}]

IMPORTANT: Use ONLY the confirmed participants list above. Do NOT extract participant names from the transcript.
    ...
    "action_items": [
      {
        ...
        "assigned_to": "string - person responsible (must be from participants list)",
...
8. IMPORTANT: Use ONLY the provided participants list [${participantsString}]. Do NOT extract names from the transcript.
9. When assigning action items, use names from the confirmed participants list only.
`;
    ...
    try {
      ...
      try {
        ...
        if (parsed.meeting_summary) {
          ...
          // Ensure participants list matches the provided list
          parsed.meeting_summary.participants = actualParticipants;
        }
        ...
        // Create a fallback structure with the actual meeting date/time and participants
      ...
      // Return a proper error structure with actual meeting date/time and participants
      const errorSummary = {
        meeting_summary: {
