...
const Pricing: React.FC = () => {
  ...
  return (
          ...
          <div className="grid md:grid-cols-3 gap-10">
            {plans.map((plan, index) => (
              <motion.div
                ...
                ${plan.highlight ? "ring-2 ring-cyan-400 bg-gradient-to-br from-white/30 to-cyan-50/30" : ""}
              `}
              >
                {/* NEON BORDER EFFECT */}
                <div
                  className={`absolute inset-0 rounded-3xl border-2 opacity-30 transition-all duration-300
                ${plan.highlight ? "border-cyan-400" : "border-blue-200 hover:border-cyan-300"}`}
                ></div>

                <h3 className="text-2xl font-bold text-cyan-700 mb-4 relative z-10">{plan.name}</h3>
                ...
                <motion.button
                  ...
                  ${plan.highlight ? "bg-cyan-600 text-white shadow-lg hover:bg-cyan-700" : "bg-white/70 text-cyan-700 hover:bg-cyan-100"}`}
                >
    ...
    //             plan.highlight ? "border-cyan-400 ring-2 ring-cyan-200" : ""
    //           }`}
    //         >
    //           <h3 className="text-2xl font-bold text-cyan-700 mb-4">{plan.name}</h3>
    ...
    //           <button className={`px-6 py-3 rounded-full font-semibold transition-all ${plan.highlight ? "bg-cyan-600 text-white hover:bg-cyan-700" : "bg-blue-100 text-cyan-700 hover:bg-blue-200"}`}>
    //             {plan.highlight ? "Get Started" : "Learn More"}
    //           </button>
    //         </motion.div>
    //       ))}
