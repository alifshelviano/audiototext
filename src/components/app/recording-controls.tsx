...
// import { Mic, Square, Circle, User, Volume2 } from 'lucide-react';
...
// export function RecordingControls({meetingId, onTranscriptAdded}: RecordingControlsProps) {
//   const { user } = useAuth();
...
//       for (let i = 0; i < dataArray.length; i++) {
//         // Only draw every other bar for better performance and visual clarity
//         if (i % 2 === 0) {
//           const barHeight = (dataArray[i] / 255) * canvas.height;

//           // Create gradient based on audio level and position
...
//             barGradient.addColorStop(0.7, '#f97316');
...
//             barGradient.addColorStop(0.7, '#059669');
...
//       canvasCtx.strokeStyle = 'rgba(156, 163, 175, 0.3)';
...
//           const {transcription} = await transcribeAudioOpenAI({audioDataUri: base64AudioData});
...
//       setIsProcessing(false);
//     }
//   }, [meetingId, user, onTranscriptAdded]);
...
//       // Process audio chunks every 2 seconds during recording
...
//     if (audioLevel > 60) return { text: 'Loud volume', color: 'text-red-600', emoji: '🎤' };
//     if (audioLevel > 30) return { text: 'Good volume', color: 'text-green-600', emoji: '🎤' };
//     if (audioLevel > 10) return { text: 'Medium volume', color: 'text-blue-600', emoji: '🎤' };
//     return { text: 'Low volume', color: 'text-gray-600', emoji: '🔇' };
//   };

//   const getVolumeAdvice = () => {
//     if (audioLevel > 60) return 'Perfect! Keep going.';
//     if (audioLevel > 30) return 'Good volume. Speak naturally.';
//     if (audioLevel > 10) return 'Getting better. Speak a bit louder.';
//     return 'Speak louder for better transcription.';
//   };
...
//             Please log in to start recording and transcribing audio.
//           </p>
//           <Button 
...
//                 width={400}
//                 height={120}
//                 className="w-full h-20 rounded-lg border border-gray-200 bg-gray-50 shadow-sm"
//               />
...
//               isRecording 
//                 ? 'bg-red-600 hover:bg-red-700 scale-110 shadow-red-200' 
...
//             <span className="text-sm text-blue-700 font-medium">Processing audio...</span>
//           </div>
//         )}
...
//               <strong>Tip:</strong> Speak clearly and maintain a consistent volume for best results
//             </p>
//           </div>
//         )}
//       </div>
                ...
                {isRecording ? (
                  <>
                    ...
                    <div className="text-xs text-gray-500">
                      {isPaused ? '⏸️ Paused' : 
                       audioLevel > 60 ? '🎤 Speaking clearly' : 
                       audioLevel > 30 ? '🎤 Good volume' : '🎤 Speak louder'}
                    </div>
                  </>
                ) : (
                  <div className="flex items-center gap-2">
