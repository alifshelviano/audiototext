...
import { generateSentimentSummary } from "@/ai/models/sentiment-analysis";
...
export function SentimentTab({ meeting }: SentimentTabProps) {
  ...
  // Add this function to generate the sentiment summary
  const getSentimentSummaryText = () => {
    const structuredSummary = getStructuredSummary();
    const emotionAnalysis = structuredSummary.emotion_analysis;


    if (!emotionAnalysis || !emotionAnalysis.overall_sentiment) {
      return "No sentiment data available for this meeting.";
    }


    // Create the required EmotionAnalysisResult object
    const emotionResults = {
      overall_sentiment: emotionAnalysis.overall_sentiment,
      overall_confidence: emotionAnalysis.overall_confidence,
      participant_emotions: emotionAnalysis.participant_emotions,
      emotional_highlights: emotionAnalysis.emotional_highlights || [],
      tension_points: emotionAnalysis.tension_points || [],
    };


    // Determine language
    const language = meeting?.language || "english"; // Default to English, adjust as needed


    return generateSentimentSummary(emotionResults, language);
  };
  ...
  const structuredSummary = getStructuredSummary();
  const sentimentSummaryText = getSentimentSummaryText();
  const emotionAnalysis = structuredSummary.emotion_analysis;
      ...
      <div className="space-y-6">
        ...
        <div className="bg-gradient-to-br from-pink-50 via-rose-50 to-red-50 rounded-2xl p-6 border border-pink-100 shadow-sm">
          ...
          <div className="bg-white/70 rounded-xl p-6 text-center">
            ...
            {/* Sentiment Summary - Moved outside the confidence bar */}
            <div className="mt-4 p-4 bg-gray-50 rounded-lg border border-gray-200">
              <p className="text-sm text-gray-700 leading-relaxed text-left">{sentimentSummaryText}</p>
            </div>
          </div>
        </div>
