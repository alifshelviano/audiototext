...
export function MeetingHeader({
  ...
  const uniqueParticipants = Array.from(
    new Set(meeting?.transcripts?.map((t: any) => t.name) || [])
  ) as string[];
  ...
  const InfoCard = ({
    ...
    <div
      className={`bg-white/70 backdrop-blur-sm rounded-xl sm:rounded-2xl p-3 sm:p-4 border border-cyan-100/50 ${onClick ? "cursor-pointer" : ""} ${className}`}
      onClick={onClick}
    >
    ...
    <>
      <div className="bg-gradient-to-br from-white to-cyan-50/20 rounded-2xl sm:rounded-3xl shadow-lg border border-cyan-100 p-4 sm:p-6 lg:p-8 mb-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-5">
          <div className="flex-1">
            <div className="flex items-center gap-2 sm:gap-3 mb-2">
              ...
              <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold bg-gradient-to-r from-cyan-600 to-teal-700 bg-clip-text text-transparent truncate">
                {meeting.name}
              </h1>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-2 text-xs sm:text-sm">
              {getStatusBadge()}
              {meeting.language && (
                <span className="px-2 py-1 bg-gray-100 rounded capitalize font-medium">
                  {meeting.language}
                </span>
              )}
        ...
        <div className="hidden md:grid md:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-5">
          <InfoCard
            icon={Calendar}
            label="Date"
            value={formatDate(meeting.time)}
          />
          <InfoCard
            icon={Clock}
            label="Time"
            value={formatTime(meeting.time)}
          />
          <InfoCard
            icon={MessageSquare}
            label="Messages"
            value={meeting.transcripts?.length || 0}
            sub={speakingStats && `Top: ${speakingStats.topSpeaker}`}
          />
          <InfoCard
            icon={Users}
            label="Participants"
        ...
        <div className="md:hidden relative mb-5" ref={dropdownRef}>
          <button
            ...
            className="w-full bg-white/70 backdrop-blur-sm rounded-xl p-3 border border-cyan-100/50 flex items-center justify-between hover:bg-white/90 transition-all"
          >
                ...
                >
                  ...
                  <div className="flex-1">
                    ...
                    <p className="font-medium text-gray-900">
                      {uniqueParticipants.length} people
                    </p>
                  </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 text-xs sm:text-sm">
            ...
            <div className="flex items-center gap-2 bg-cyan-50/70 px-3 py-2 rounded-xl border border-cyan-100">
              ...
              <span className="text-gray-700">
                <span className="font-semibold text-cyan-600">
                  {speakingStats.topSpeaker}
                </span>{" "}
                most active (
                {Math.round(
                  (speakingStats.messageCount / speakingStats.totalMessages) *
                    100
                )}
                %)
              </span>
          ...
          {meeting.isPublic !== undefined && (
            <div className="flex items-center gap-2 bg-cyan-50/70 px-3 py-2 rounded-xl border border-cyan-100">
              ...
              <span className="font-semibold text-cyan-600">
                {meeting.isPublic ? "Public" : "Private"} Meeting
              </span>
            </div>
          )}
        </div>
      ...
      {showParticipants && (
        ...
        >
          <div
            ref={participantsRef}
            className="bg-white rounded-2xl sm:rounded-3xl shadow-2xl border border-gray-200 w-full max-w-md max-h-[85vh] overflow-hidden"
          ...
          >
            ...
            <div className="p-4 sm:p-6 border-b border-gray-100 bg-gradient-to-r from-cyan-50 to-orange-50">
              ...
              <Badge className="bg-white text-cyan-700 border border-cyan-200 text-xs sm:text-sm">
            ...
            <div className="p-4 sm:p-6 space-y-3 max-h-96 overflow-y-auto">
              {uniqueParticipants.map((name, i) => {
                const count =
                  meeting.transcripts?.filter((t) => t.name === name).length ||
                  0;
                const isTop = speakingStats?.topSpeaker === name;
                return (
                  <div
                    key={i}
                    className="flex items-center gap-3 p-3 rounded-xl hover:bg-gray-50 transition-all border border-gray-100"
                  >
            ...
            <div className="p-4 sm:p-6 border-t border-gray-100 bg-gray-50">
              <Button
                className="w-full bg-cyan-600 hover:bg-cyan-700 text-white font-semibold rounded-xl h-11"
                onClick={() => setShowParticipants(false)}
              >
                Close
              </Button>
...
//   const getMeetingDuration = () => {
//     if (!meeting.transcripts?.length) return "0 min";
//     const timestamps = meeting.transcripts
//       .map((t) => new Date(t.createdAt).getTime())
//       .filter((t) => !isNaN(t));
...
//     );
//     return duration > 0 ? `${duration} min` : "Live";
//   };
...
//         {/* === DROPDOWN: Meeting Info (Date, Time, Messages, Participants) === */}
//         <div className="relative mb-5" ref={dropdownRef}>
//           <button
//             onClick={() => setShowInfoDropdown(!showInfoDropdown)}
//             className="w-full bg-white/70 backdrop-blur-sm rounded-xl sm:rounded-2xl p-3 sm:p-4 border border-cyan-100/50 flex items-center justify-between hover:bg-white/90 transition-all"
//           >
//             <div className="flex items-center gap-3">
//               <div className="p-2 bg-cyan-50 rounded-lg">
//                 <Calendar className="w-4 h-4 text-cyan-600" />
...
//                     <p className="font-medium text-gray-900">{formatDate(meeting.time)}</p>
//                   </div>
//                 </div>
...
//                     <p className="font-medium text-gray-900">{formatTime(meeting.time)}</p>
//                   </div>
//                 </div>
...
//                 most active (
//                 {Math.round(
//                   (speakingStats.messageCount / speakingStats.totalMessages) * 100
//                 )}
//                 %)
//               </span>
//             </div>
...
//               {uniqueParticipants.map((name, i) => {
//                 const count =
//                   meeting.transcripts?.filter((t) => t.name === name).length || 0;
//                 const isTop = speakingStats?.topSpeaker === name;
//                 return (
//                   <div
//                     key={i}
...
//           {/* Date */}
//           <div className="bg-white/70 backdrop-blur-sm rounded-xl sm:rounded-2xl p-3 sm:p-4 border border-cyan-100/50">
//             <div className="flex items-center gap-2 sm:gap-3">
//               <div className="p-1.5 sm:p-2 bg-cyan-50 rounded-lg">
//                 <Calendar className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-cyan-600" />
//               </div>
//               <div>
...
//             <div className="flex items-center gap-2 sm:gap-3">
//               <div className="p-1.5 sm:p-2 bg-cyan-50 rounded-lg">
//                 <MessageSquare className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-cyan-600" />
//               </div>
//               <div>
...
//           {/* Participants */}
//           <div
//             className="bg-white/70 backdrop-blur-sm rounded-xl sm:rounded-2xl p-3 sm:p-4 border border-cyan-100/50 cursor-pointer"
//             onClick={() => setShowParticipants(true)}
//           >
//             <div className="flex items-center gap-2 sm:gap-3">
...
//       {/* Participants Modal - Fully Responsive */}
//       {showParticipants && (
//         <div
//           className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50"
//           onClick={() => setShowParticipants(false)}
