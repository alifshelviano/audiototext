...
export function TranscriptList({ transcripts, visibleCount, onLoadMore, onShowLess }: TranscriptListProps) {
  ...
  return (
    <>
      {transcripts.length > visibleCount && (
        ...
        <div key={index} className="flex items-start gap-4 p-4 rounded-xl hover:bg-gradient-to-r hover:from-blue-50/50 hover:to-transparent transition-all duration-200 group">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-3 mb-2">
              ...
              <span className="text-xs text-gray-500 bg-gray-100 px-2.5 py-1 rounded-full font-medium">{new Date(transcript.createdAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</span>
            </div>
