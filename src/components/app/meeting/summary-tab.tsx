...
import { Download, Mail, RefreshCw, FileText, ChevronDown, Users, X } from "lucide-react";
import { SummaryContent } from "@/components/app/meeting/summary-content";
import { ExportService } from "@/lib/export-service";
import type { MeetingData, MeetingSummary } from "@/models/Meeting";
...
export function SummaryTab({ meeting, isAnalyzing, onReanalyze }: SummaryTabProps) {
  ...
  const validRecipientCount = useMemo(() => {
    return allRecipients.filter(recipient => 
      recipient.isValid && selectedRecipients.has(recipient.email)
    ).length;
  ...
  useEffect(() => {
    if (showEmailModal) {
      ...
      const validEmails = allRecipients
  ...
  const handleSendEmails = async () => {
    ...
    try {
      ...
      const selectedValidEmails = allRecipients
        .filter(recipient => recipient.isValid && selectedRecipients.has(recipient.email))
        .map(recipient => recipient.email);
  ...
  return (
    <div className="p-6 overflow-y-auto">
      <div className="flex justify-between items-center mb-6">
        <h3 className="text-2xl font-bold bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">Meeting Summary</h3>
          ...
          <div className="bg-white rounded-xl shadow-lg max-w-2xl w-full max-h-[90vh] overflow-hidden">
                      ...
                      {allRecipients.map((recipient, index) => (
                        <div 
                          ...
                          className={`flex items-center justify-between p-3 rounded-lg border transition-colors cursor-pointer ${
                            ...
                            !recipient.isValid ? "opacity-60 cursor-not-allowed" : "cursor-pointer"
                          }`}
                        ...
                        >
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center space-x-2 mb-1">
                                <span className={`font-medium truncate ${
                                  recipient.isValid ? "text-gray-900" : "text-yellow-700"
                                }`}>
                                  {recipient.name}
                                </span>
                                {recipient.isParticipant && (
                                  <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${
                                    recipient.source === 'participant' 
                                      ? 'bg-green-100 text-green-800'
                                      : recipient.source === 'transcript'
                                      ? 'bg-blue-100 text-blue-800'
                                      : 'bg-gray-100 text-gray-800'
                                  }`}>
                                    {recipient.source === 'participant' ? 'Participant' : 
                                     recipient.source === 'transcript' ? 'From Transcript' : 'Manual'}
                                  </span>
                                )}
                              </div>
                              <p className={`text-sm truncate ${
                                recipient.isValid ? "text-gray-600" : "text-yellow-600"
                              }`}>
                                {recipient.email}
                                {!recipient.isValid && recipient.source === 'transcript' && (
                                  <span className="text-xs text-yellow-600 ml-2">(email generated from name)</span>
                                )}
                              </p>
                            </div>
                          </div>
                          ...
                          {!recipient.isValid && (
                            <span className="text-xs text-yellow-600 bg-yellow-100 px-2 py-1 rounded whitespace-nowrap ml-2">
                              {recipient.source === 'transcript' ? 'Needs real email' : 'Invalid email'}
                            </span>
                          )}
                        </div>
                      ))}
            ...
            <div className="flex gap-3 p-6 border-t border-gray-200 bg-gray-50">
              ...
              <Button
                ...
                disabled={isSendingEmail || validRecipientCount === 0}
