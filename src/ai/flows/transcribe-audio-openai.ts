// ai/flows/transcribe-audio-openai.ts
"use server";
...
const TranscribeAudioOpenAIInputSchema = z.object({
  audioDataUri: z.string().describe("The audio file data as a data URI that must include a MIME type and use Base64 encoding. Expected format: 'data:<mimetype>;base64,<encoded_data>'."),
  language: z.enum(["english", "indonesian", "korean"]).optional().describe("The language of the audio for transcription"),
});
...
export async function transcribeAudioOpenAI(input: TranscribeAudioOpenAIInput): Promise<TranscribeAudioOpenAIOutput> {
...
const transcribeAudioOpenAIFlow = ai.defineFlow(
  ...
  async (input) => {
    ...
    const whisperLanguage = language ? languageMap[language] : "en";
    ...
    formData.append("language", whisperLanguage); // Use the provided language

    console.log(`Sending audio to Elice Whisper with language: ${language} (${whisperLanguage})`);

    const response = await fetch("https://mlapi.run/805a20fb-b66b-4b7c-84fb-079c12b76937/v1/audio/transcriptions", {
      method: "POST",
      headers: {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Elice API error: ${response.status} ${response.statusText} - ${errorText}`);
    }
    ...
    if (transcription) {
      ...
      for (let i = 0; i < words.length; i++) {
        if (i > 0 && words[i] === words[i - 1]) {
          repeatCount++;
          if (repeatCount < 3) {
            // Allow up to 2 repeats, remove beyond that
