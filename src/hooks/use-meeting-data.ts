...
export function useMeetingData(meetingId: string | null) {
  ...
  useEffect(() => {
    const autoAnalyzeIfNeeded = async () => {
      if (meeting?.transcripts?.length && meeting.transcripts.length > 0) {
        console.log("🔍 Checking if auto-analyze is needed...");
        const shouldAnalyze = await shouldAutoAnalyze(meeting.id);
        console.log(`📊 Should auto-analyze: ${shouldAnalyze}`);


        if (shouldAnalyze && !meeting.summary) {
