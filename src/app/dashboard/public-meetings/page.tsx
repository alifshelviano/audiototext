...
export default function PublicMeetingsPage() {
  ...
  const getLanguageColor = (language: string) => {
    const colors: { [key: string]: string } = {
      ...
      spanish: "bg-yellow-100 text-yellow-700 border-yellow-200",
      french: "bg-blue-100 text-blue-700 border-blue-200",
      german: "bg-yellow-100 text-yellow-700 border-yellow-200",
      japanese: "bg-red-100 text-red-700 border-red-200",
      ...
      <div key={meeting.id} className="group bg-white rounded-2xl shadow-lg border border-gray-100 p-6 hover:shadow-2xl transition-all duration-300 hover:scale-105 hover:border-blue-200">
        ...
        <div className="flex items-start justify-between mb-4">
          <div className="flex-1 min-w-0">
            <h3 className="font-bold text-xl text-gray-800 pr-2 line-clamp-2 group-hover:text-blue-600 transition-colors">{meeting.name}</h3>
          </div>
        ...
        <div className="space-y-3 mb-6">
          <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl">
            ...
            <div className={`px-3 py-1 rounded-full text-xs font-medium ${datetime.status === "upcoming" ? "bg-blue-100 text-blue-700" : "bg-gray-100 text-gray-600"}`}>{datetime.status === "upcoming" ? "Upcoming" : "Past"}</div>
          </div>
  ...
  const renderEmptyState = () => (
    <div className="text-center py-16 border-2 border-dashed border-gray-300 rounded-2xl bg-gradient-to-br from-gray-50 to-blue-50/30">
      ...
      <h3 className="text-2xl font-bold text-gray-800 mb-3">No Public Meetings Available</h3>
      <p className="text-gray-500 mb-8 max-w-md mx-auto text-lg">There are no public meetings happening right now. Check back later or create your own meeting to get started.</p>
    ...
    <DashboardLayout>
      <div className="mb-8">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6 mb-8">
          <div className="flex-1">
            <h1 className="text-4xl font-bold bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent mb-3">Public Meetings</h1>
            <p className="text-xl text-gray-600 max-w-2xl">Browse and join ongoing public meetings. No login required to participate in these open discussions.</p>
          </div>
          <Button
            onClick={fetchMeetings}
            disabled={isLoading}
            variant="outline"
            className="flex items-center gap-3 px-6 py-3 border-2 border-gray-200 hover:border-blue-300 rounded-xl hover:bg-blue-50 transition-all duration-300 disabled:opacity-50"
          >
      ...
      {!isLoading && meetings.length > 0 && (
        <div className="mb-6 flex items-center justify-between">
          <p className="text-gray-600 text-lg">
            Showing <span className="font-semibold text-blue-600">{filteredMeetings.length}</span> of <span className="font-semibold text-gray-800">{meetings.length}</span> public meetings
          </p>
          {searchQuery && (
