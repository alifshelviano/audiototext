...
export default function PublicMeetingsPage() {
  ...
  const formatDateTime = (dateString: string) => {
    ...
    if (date > now) {
      return `Starts ${date.toLocaleDateString()} at ${date.toLocaleTimeString([], options)}`;
    } else {
  ...
  const getLanguageFlag = (language: string) => {
    const flags: { [key: string]: string } = { english: "🇺🇸", indonesian: "🇮🇩", korean: "🇰🇷" };
  ...
  const renderMeetingCard = (meeting: Meeting) => (
    <div key={meeting.id} className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 hover:shadow-md transition-shadow">
      ...
      <div className="space-y-2 mb-4">
        <div className="flex items-center gap-2 text-sm text-gray-500"><Clock className="w-4 h-4" /><span>{formatDateTime(meeting.time)}</span></div>
        <div className="flex items-center gap-2 text-sm text-gray-500"><span className="text-lg">{getLanguageFlag(meeting.language)}</span><span className="capitalize">{meeting.language}</span></div>
      </div>
  ...
  if (isLoading) {
    ...
    content = (
      <div className="text-center py-16 border-2 border-dashed border-gray-300 rounded-xl bg-white">
        ...
        <p className="text-gray-500 mb-6 max-w-md mx-auto">There are no public meetings happening right now. Check back later or create your own meeting.</p>
