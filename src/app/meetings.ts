...
// export async function getMeetings({ userId }: { userId?: string }): Promise<Array<{id: string; name: string; time: string}>> {
//   try {
//     const client = await clientPromise;
//     const db = client.db();
//     const meetingsCollection = db.collection<Meeting>('meetings');
...
export async function generateMeetingSummary({ meetingId, transcripts }: { meetingId: string; transcripts: any[] }): Promise<{ success: boolean; summary?: any; error?: string }> {
  try {
    if (!transcripts || transcripts.length === 0) {
      return { success: false, error: "No transcripts available" };
    }
