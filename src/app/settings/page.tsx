                  ...
                  <div className="space-y-2">
                    ...
                    {passwordData.newPassword && (
                      <div className="space-y-2 p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
                        ...
                        <div className="space-y-1 text-sm">
                          {[
                            { label: "At least 8 characters", met: passwordValidation.requirements.minLength },
                            { label: "One uppercase letter", met: passwordValidation.requirements.hasUpperCase },
                            { label: "One lowercase letter", met: passwordValidation.requirements.hasLowerCase },
                            { label: "One number", met: passwordValidation.requirements.hasNumbers },
                            { label: "One special character", met: passwordValidation.requirements.hasSpecialChar },
                          ].map((req, index) => (
                            <div key={index} className={`flex items-center gap-2 ${req.met ? "text-green-600" : "text-gray-500"}`}>
                              {req.met ? <CheckCircle className="w-4 h-4" /> : <AlertCircle className="w-4 h-4" />}
                              {req.label}
